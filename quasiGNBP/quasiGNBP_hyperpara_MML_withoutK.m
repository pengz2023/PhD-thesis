...
% quasiGNBP prior and Lasso estimates of diagonal elements of the precision
% matrix
...
%     Diag:                Lasso estimates of diagonal elements of the precision matrix;



function [samples_a_lambda_sq, samples_b_xi_sq, a_MML, b_MML, Diag] = quasiGNBP_hyperpara_MML_withoutK(Y, a_lambda_sq,b_xi_sq,EM_maxiter) 

% Y: n by p matrix, n: sample size
[n,p] = size(Y);  
S = Y'*Y;
...
% Empirical estimates of C(i,i)
sigmasq = zeros(p,1);

% paralell computing
parfor i = 1:p
    position = ind_noi_all(:,i)';
    [B,FitInfo] = lasso(Y(:,position), Y(:,i), 'CV', 10);
    betahat = B(:,FitInfo.IndexMinMSE); 
    df = FitInfo.DF(FitInfo.IndexMinMSE);
    sigmasq(i) = (1/(n- min(df,n-1) ))*sum( (Y(:,i)-Y(:,position)*betahat).^2 );
end
Diag = 1./sigmasq;
